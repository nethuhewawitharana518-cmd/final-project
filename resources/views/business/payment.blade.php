...
<div class="d-flex">
    ...
    <div class="main-content flex-grow-1">
        <div class="page-header">
            ...
            <a href="{{ route('business.subscription') }}" class="btn btn-outline-danger btn-sm px-3">Cancel</a>
        </div>
            ...
            <div class="card border-0 shadow-sm bg-white p-4 rounded-3">
                ...
                <div class="p-3 border rounded-3 bg-light mb-4">
                    <div class="d-flex justify-content-between mb-2">
                        ...
                        <span class="fw-bold text-dark">{{ $planDetails['name'] ?? ucfirst($plan) }}</span>
                    </div>
                    <div class="d-flex justify-content-between">
                        <span class="text-muted">Renewal Interval:</span>
                        <span class="text-dark">{{ $planDetails['duration'] ?? 30 }} Days</span>
                    </div>
                    <hr>
                    <div class="d-flex justify-content-between fs-5 fw-bold">
                        <span class="text-dark">Amount Due:</span>
                        <span class="text-success">Rs. {{ number_format($planDetails['price'] ?? 0) }}</span>
                    </div>
                </div>

                <form action="{{ route('business.subscription.post') }}" method="POST">
                    @csrf
