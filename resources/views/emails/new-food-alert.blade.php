...
<body>
    ...
    <div class="container">
        ...
        <div class="content">
            <span class="restaurant-tag">🏪 {{ $food->business->business_name }}</span>
            
            @if($embeddedImage)
                <div style="margin-bottom: 25px; text-align: center;">
                    <img src="{{ $embeddedImage }}" alt="{{ $food->name }}" style="max-width: 100%; max-height: 240px; border-radius: 8px; object-fit: cover; display: inline-block; border: 1px solid #2d2d2d;">
                </div>
            @endif

            <h2 class="food-title">{{ $food->name }}</h2>
            <p class="description">{{ $food->description ?: 'No description provided.' }}</p>
            ...
            <div style="text-align: center; margin-bottom: 30px; width: 100%;">
                <span style="color: #777777; text-decoration: line-through; font-size: 16px; margin-right: 15px; display: inline-block;">
                    Rs. {{ number_format($food->original_price, 2) }}
                </span>
                <span style="color: #10b981; font-size: 24px; font-weight: 800; display: inline-block;">
                    Rs. {{ number_format($food->discount_price, 2) }}
                </span>
            </div>

            <div class="btn-container">
                <a href="{{ route('food.browse') }}" class="btn">View Deal Now</a>
            </div>
        </div>
        <div class="footer">
