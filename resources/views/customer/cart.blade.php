    ...
    @if(empty($cart))
        <div class="card border-0 shadow-sm p-5 text-center bg-white rounded-3">
            <div class="py-5">
                ...
                <a href="{{ route('food.browse') }}" class="btn btn-success px-4 py-2 rounded-pill fw-semibold shadow-sm">
                    Browse Deals
                </a>
            </div>
        </div>
            ...
            <div class="col-lg-8">
                <div class="card border-0 shadow-sm bg-white rounded-3 p-4">
                    <div class="d-flex justify-between align-items-center mb-3">
                        <span class="text-dark fw-bold">Items from: <span class="text-success">{{ reset($cart)['business_name'] }}</span></span>
                                    ...
                                    <tr>
                                        <td>
                                            <div class="d-flex align-items-center gap-3">
                                                @if($item['image'])
                                                    <img src="{{ str_starts_with($item['image'], 'assets/') ? asset($item['image']) : asset('storage/' . $item['image']) }}" alt="{{ $item['name'] }}" class="rounded-3" style="width: 60px; height: 60px; object-fit: cover;">
                                                @else
                                                ...
                                                <div class="text-start">
                                                    ...
                                                    <small class="text-muted d-block">{{ $item['business_name'] }}</small>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="text-center">
                                            <span class="text-muted small text-decoration-line-through d-block">Rs. {{ number_format($item['original_price'], 2) }}</span>
                                            <span class="fw-bold text-success">Rs. {{ number_format($item['discount_price'], 2) }}</span>
                                        </td>
                                        <td class="text-center">
                                        ...
                                        <td class="text-end fw-bold text-dark">
                                            Rs. <span class="item-total-val" id="total_{{ $id }}">{{ number_format($item['discount_price'] * $item['quantity'], 2) }}</span>
                                        </td>
                                        <td class="text-center">
            ...
            <div class="col-lg-4">
                <div class="card border-0 shadow-sm bg-white rounded-3 p-4">
                    ...
                    <div class="d-flex justify-content-between mb-3 text-muted">
                        ...
                        <span class="fw-bold text-dark">Rs. <span id="cart-subtotal">{{ number_format(array_sum(array_map(fn($i) => $i['discount_price'] * $i['quantity'], $cart)), 2) }}</span></span>
                    </div>
                    <div class="d-flex justify-content-between mb-4 text-muted">
                        <span>Rescue Discount</span>
                        <span class="fw-bold text-success">- Rs. {{ number_format(array_sum(array_map(fn($i) => ($i['original_price'] - $i['discount_price']) * $i['quantity'], $cart)), 2) }}</span>
                    </div>
                    ...
                    <div class="d-flex justify-content-between mb-4 fs-5 text-dark fw-bold">
                        <span>Total Due</span>
                        <span>Rs. <span id="cart-total">{{ number_format(array_sum(array_map(fn($i) => $i['discount_price'] * $i['quantity'], $cart)), 2) }}</span></span>
                    </div>

                    <a href="{{ route('customer.checkout') }}" class="btn btn-success w-100 py-3 rounded-pill fw-semibold shadow-sm mb-3">
                        Proceed to Checkout <i class="fa fa-arrow-right ms-1"></i>
                    </a>
                    
                    <a href="{{ route('food.browse') }}" class="btn btn-outline-secondary w-100 py-2 rounded-pill fw-semibold">
                        Continue Shopping
                    </a>
                </div>
            </div>
